...
for1:	cmp r5, r2 @ Se compara j con N
		...
		bl MAX_MOD @ Salta a MAX_MOD y en lr se guarda la dir de mem de la siguiente instrucción
		ldr r6, =ind @ r6 = &ind
		ldr r7, [r1,r0,lsl#2] @ r7 = A(ind)
		str r7, [r4,r5,lsl#2] @ Se escribe A(ind) en la pos de mem de B(j)
		mov r7, #0
		str r7, [r1,r0,lsl#2] @ Se escribe un 0 en A(ind)
		add r5, r5, #1 @ j++
		b for1
...
MAX_MOD:
		push {r4-r8,fp,lr} @ Se guardan los valores de los registros que se usan en la función MAX
...
if:		cmp r8, r6 @ Comparo A(i) con max
		...
		bl ACT
		mov r6, r2 @ max = A(i)
		mov r7, r3 @ Se copia/guarda el valor de i
		ldr r1, =A @ Se restaura el valor de r1
		mov r2, #N @ Se restaura el valor de r2
endif:	add r4, r4, #1 @ i++
		b for
...
ACT:	@ r0 = &max,r1 = &ind,r2 = A(i),r3 = i
		...
		mov r6,r2 @ r6 = A(i)
		mov r7,r3 @ r7 = i
		str r6,[r4] @ Se escribe el valor de max en memoria (max = A(i))
		str r7,[r5] @ Se escribe el valor de i en memoria (ind = i)
		sub sp,fp,#20
		pop {r4-r7,fp,lr}
		mov pc,lr
		.end
